...
def test_check_valid_config(config: Mapping):
    outcome = DestinationSftpCsv().check(logging.getLogger("airbyte-destination"), config)
    assert outcome.status == Status.SUCCEEDED


def test_check_invalid_config(config):
    outcome = DestinationSftpCsv().check(logging.getLogger("airbyte-destination"), {**config, "destination_path": "/doesnotexist"})
